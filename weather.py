...
# get yours at https://developer.forecast.io
...
def get_locations():
    memphis = {'city': 'Memphis',
               'region': 'Tennessee',
               'country': 'US',
               'loc': '35.176546,-89.850054',
               'postal': '38134'
               }
    gtown = {'city': 'Germantown',
             'region': 'Tennessee',
             'country': 'US',
             'loc': '35.075158, -89.777019',
             'postal': '38138'}
    return [memphis, gtown]

def get_forecast_url(api_key, location, units='us'):
    accepted_units = ['us', 'si']
    if units not in accepted_units:
        raise ValueError('I do not recognize the unit provided!')
    base_url = 'https://api.forecast.io/forecast'
    lat_lng = location['loc']
    url = "{0}/{1}/{2}?units={3}".format(base_url,
                                         api_key,
                                         lat_lng,
                                         units)
    return url
...
def get_wx():
  ...
  try:
    if 'loc' in location:
      wx = json.load(urllib2.urlopen('https://api.forecast.io/forecast/' + api_key + '/' + location['loc'] + '?units=' + units))
    else:
      return False
  except urllib2.HTTPError:
    return False
